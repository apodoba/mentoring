...
public class JMSMessageListener implements MessageListener {
	...
	private Object execute(MapMessage message) throws JMSException {
		...
		case TICKETS:
			...
			Date time = new Date((long) params.get("time"));
			...
			for(Ticket ticket: tickets){
				...
				transport.put("price", ticket.getPrice().doubleValue());
		...
		case CHECK_STATUS:
			id = (long) params.get("id");
			response = ticketService.changeStatusToProccessPayment(id);
			break;
		case ROLLBACK_PAYMENT:
			id = (long) params.get("id");
			response = ticketService.rollbackToBlock(id);
			break;
		}
