...
public class TicketDaoImpl implements TicketDao{
	...
	public Map<String, List<Long>> getCinemaScheduler() {
		...
		SQLQuery query = sessionFactory.getCurrentSession().createSQLQuery("SELECT film, session FROM cinema.ticket group by film, session;");
	...
	@Override
	public void unBlockTickets() {
		String unblockSQL = "update ticket set status = 0 where status = 1 and TIMESTAMPDIFF(day,block_time, now())*24*60 + MOD( TIMESTAMPDIFF(hour,block_time,now()), 24)*60 + "
				+ "MOD( TIMESTAMPDIFF(minute,block_time,now()), 60)>1";
		sessionFactory.getCurrentSession().createSQLQuery(unblockSQL).executeUpdate();
	}

	@Override
	public boolean changeStatusToProccessPayment(long ticketId) {
		Ticket ticket = (Ticket) sessionFactory.getCurrentSession().get(Ticket.class, ticketId);
	...
	@Override
	public boolean rollbackToBlock(long ticketId) {
		Ticket ticket = (Ticket) sessionFactory.getCurrentSession().get(Ticket.class, ticketId);
		if(ticket.getStatus() == 5){
			ticket.setStatus(1);
			sessionFactory.getCurrentSession().merge(ticket);
